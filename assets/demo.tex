...
\begin{tabularx}{\linewidth}{@{}lcc@{}}
	...
	\multicolumn{3}{l}{{\footnotesize \textsuperscript{b} Recommended for Faculty of Science students.}} \\
	\multicolumn{3}{l}{{\footnotesize \textsuperscript{c} Recommended for Lassonde students.}}
	\endlastfoot
	%%%%% START %%%%%
...
\subsection{\texttt{bm}}
The \texttt{bm} package gives you the \texttt{\textbackslash bm\{\}} command, which makes a math symbol bold while keeping it italic. This is a common way to write vectors and matrices in science and engineering, since \texttt{\textbackslash mathbf} would make the letter upright:
...
The \texttt{array} package lets you apply formatting to an entire column at once. The \texttt{>{\ldots}} syntax goes before the column letter in your column definition and applies a command to every cell in that column. The example below uses it to centre and wrap text in two fixed-width columns. The template defines shorthand column types \texttt{P\{\}}, \texttt{Q\{\}}, and \texttt{S\{\}} as \texttt{ragged2e}-aware equivalents of \texttt{p\{\}} for left, centre, and right alignment respectively:
...
\noindent\colorbox{themecolour!15}{\parbox{\linewidth}{\small \texttt{tabularx} is used internally by \texttt{ltablex} and is not the preferred table environment in this template. Use \texttt{ltablex} for all tables. The example below is shown for reference only.}}\vspace{0.5\baselineskip}
...
\begin{table}[H]
	\centering
	\caption{Auto-width table using \texttt{tabularx} with \texttt{ragged2e} column types.}
	\label{tab:tabularx}
