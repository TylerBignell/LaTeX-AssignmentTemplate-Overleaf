...
The template also defines three \texttt{ragged2e}-aware column types built on \texttt{X}: \texttt{L} (ragged right), \texttt{C} (centred), and \texttt{R} (ragged left). These should be preferred over plain \texttt{X} for columns containing wrapped text, as they allow hyphenation and produce more even line lengths.
